...
class VendasService {
  async processarVendaPDV(data, userId) {
    ...
    try {
      ...
      for (const item of itens) {
        ...
        console.log(`[VENDAS_LOG] Verificando peça ${item.pecaId}:`, {
          status: peca?.status,
          sacolinhaIdPeca: peca?.sacolinhaId,
          sacolinhaIdReq: sacolinhaId
        });

        // Se o item já está vendido mas pertence à mesma sacolinha, nós pulamos (já foi processado)
        if (
          peca &&
          peca.status === "VENDIDA" &&
          sacolinhaId &&
          peca.sacolinhaId === parseInt(sacolinhaId)
        ) {
          continue;
        }
      ...
      if (itens.length === 0) {
        // Se não sobrou nada para vender (ex: todos já foram pagos na sacolinha), apenas comita e retorna o último pedido se existir
        await t.commit();
        return { message: "Itens já processados anteriormente", sacolinhaId };
      }
      ...
      for (const item of itens) {
        ...
        if (peca.tipo_aquisicao === "PERMUTA" && peca.fornecedorId) {
          const fornecedor = await Pessoa.findByPk(peca.fornecedorId, {
            transaction: t,
          });

          const comissaoPercent =
            fornecedor && fornecedor.comissao_padrao
      ...
      for (const pag of pagamentos) {
        ...
        if (pag.metodo === "DINHEIRO") {
          const novoTotal =
            parseFloat(caixaAberto.total_entradas_dinheiro) +
            parseFloat(pag.valor);
          await caixaAberto.update(
            { total_entradas_dinheiro: novoTotal },
            { transaction: t },
          );
        }
        ...
        if (
          clienteId &&
          !["CREDITO_LOJA", "VOUCHER_PERMUTA"].includes(pag.metodo)
        ) {
      ...
      if (sacolinhaId) {
        ...
        console.log(
          `Sale linked to sacolinha ${sacolinhaId}. Keeping it open.`,
        );
      }
      ...
      try {
        ...
        console.error(
          "[VendasService] Failed to sync sales to Ecommerce:",
          syncErr.message,
        );
      }
      // -------------------------
        ...
        if (clienteId) {
          ...
          if (cliente && cliente.telefone_whatsapp) {
            const listaProdutos = itensResumo
              .map((i) => `👗 ${i.nome} - R$ ${parseFloat(i.valor).toFixed(2)}`)
              .join("\n");
            const msgCashback =
              valorCashback > 0
            ...
            /*
                        ...
                        */
            console.log(
              "Notificação de pós-venda desativada conforme solicitação.",
            );
          }
        }
      } catch (msgErr) {
  ...
  async excluirSacolinha(id) {
    ...
    try {
      ...
      await t.commit();
      return {
        message: "Sacolinha excluída permanentemente e itens liberados.",
      };
    } catch (err) {
      await t.rollback();
      throw err;
  ...
  async liberarItensSacolinha(sacolinhaId, transaction) {
    ...
    for (const peca of pecas) {
      ...
      if (peca.codigo_etiqueta && peca.codigo_etiqueta.endsWith(suffix)) {
        ...
        if (pecaPai) {
          await pecaPai.update(
            { quantidade: pecaPai.quantidade + peca.quantidade },
            { transaction },
          );
  ...
  async adicionarItemSacolinha(sacolinhaId, pecaId, preco = null) {
    ...
    if (sacolinha.status !== "ABERTA")
      throw new Error("Só é possível adicionar itens em sacolinhas abertas");
    ...
    if (
      peca.status === "RESERVADA_SACOLINHA" &&
      peca.sacolinhaId !== parseInt(sacolinhaId)
    ) {
  ...
  async atualizarPrecoItemSacolinha(sacolinhaId, pecaId, novoPreco) {
    ...
    if (!sacolinha) throw new Error("Sacolinha não encontrada");
    if (sacolinha.status !== "ABERTA")
      throw new Error("Só é possível alterar preços em sacolinhas abertas");

    const peca = await Peca.findByPk(pecaId);
    if (!peca) throw new Error("Peça não encontrada");
    if (peca.sacolinhaId !== parseInt(sacolinhaId))
      throw new Error("Peça não pertence a esta sacolinha");
  ...
  async removerItemSacolinha(sacolinhaId, pecaId) {
    ...
    if (!sacolinha) throw new Error("Sacolinha não encontrada");
    if (sacolinha.status !== "ABERTA")
      throw new Error("Só é possível remover itens de sacolinhas abertas");
    ...
    if (peca.codigo_etiqueta && peca.codigo_etiqueta.endsWith(suffix)) {
      ...
      if (pecaPai) {
        ...
        return {
          message: "Item devolvido ao lote original e removido da sacolinha",
        };
      }
    }
  ...
  async processarDevolucao(pecaId, userId) {
    ...
    try {
      const peca = await Peca.findByPk(pecaId);
      if (!peca || peca.status !== "VENDIDA")
        throw new Error("Peça não está vendida");
  ...
  async getPedidos(filters = {}) {
    ...
    if (filters.inicio && filters.fim) {
      whereClause.data_pedido = {
        [Op.between]: [new Date(filters.inicio), new Date(filters.fim)],
      };
    }
          ...
          include: [
            {
              model: Peca,
              as: "peca",
              attributes: ["descricao_curta", "codigo_etiqueta"],
              include: [
    ...
    return pedidos.map((p) => ({
      ...
      itens: p.itens.map((i) => ({
        ...
        fornecedor:
          i.peca && i.peca.fornecedor ? i.peca.fornecedor.nome : "LOJA",
